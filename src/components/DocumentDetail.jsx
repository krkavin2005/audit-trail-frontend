...
const DocumentDetail = (props) => {
    const { doc, closeModal, handleView, fetch } = props;
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black/60 backdrop-blur-sm z-50">
            <div className="bg-grey-900 border border-slate-800 rounded-xl p-6 w-[600px] shadow-2xl shadow-indigo-900/30">
                <div className="flex justify-between items-center mb-6">
                    <div className="flex">
                        <h2 onClick={() => setActiveTab("detail")} className={`text-xl font-bold ${activeTab === "detail" ? "text-white" : "text-slate-400"} cursor-pointer mr-5`}>Detail</h2>
                        <span>|</span>
                        <h2 onClick={() => setActiveTab("history")} className={`text-xl font-bold ${activeTab === "history" ? "text-white" : "text-slate-400"} cursor-pointer ml-5`}>History</h2>
                    </div>
